...
if [[ "${PV}" == 9999* ]]; then
	...
	GENTOO_ADDITIONS_REPO_URL="
		git://github.com/kisamehoshigake/tlp-gentoo-additions.git
		https://github.com/kisamehoshigake/tlp-gentoo-additions.git
	"

	GENTOO_ADDITIONS_REMOTE_REF="refs/heads/master"
...
MY_README_URI="https://github.com/kisamehoshigake/tlp-portage/blob/maint/README.rst"
...
src_unpack() {
	if [[ "${PV}" == 9999* ]]; then
		...
		git-r3_fetch \
			"${GENTOO_ADDITIONS_REPO_URI}" "${GENTOO_ADDITIONS_REMOTE_REF}"
		git-r3_checkout \
			"${GENTOO_ADDITIONS_REPO_URI}" "${WORKDIR}/gentoo-additions"
	else
		default
	fi
}
